...
namespace QTPFS {
	...
	public:
		...
		void SetTableIndex(unsigned int n) { tableIndex = n; }
		void SetHeapIndex(unsigned int n) { heapIndex = n; }
		unsigned int GetNodeNumber() const { return nodeNumber; }
		unsigned int GetTableIndex() const { return tableIndex; }
		...
		virtual unsigned int GetNeighbors(const NodeLayer&, std::vector<INode*>&) = 0;
		virtual const std::vector<INode*>& GetNeighbors(const NodeLayer&) = 0;
		virtual bool UpdateNeighborCache(const NodeLayer&) = 0;
	...
	public:
		...
		unsigned int GetNeighbors(const NodeLayer&, std::vector<INode*>&);
		const std::vector<INode*>& GetNeighbors(const NodeLayer&);
		bool UpdateNeighborCache(const NodeLayer&);
